class EnergySalesDAO:
    ...
    def insert_energy_sales(self, sale):
        cur = self.mysql.connection.cursor()
        cur.execute(
            "INSERT INTO energy_sales (station_id, energy_sold, price_per_kWh, timestamp) VALUES (%s, %s, %s, %s)",
    ...
    def update_energy_sales(self, sale_id, sale):
        cur = self.mysql.connection.cursor()
        cur.execute(
            "UPDATE energy_sales SET station_id = %s, energy_sold = %s, price_per_kWh = %s, timestamp = %s WHERE id = %s",

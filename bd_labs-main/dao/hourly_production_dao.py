class HourlyProductionDAO:
    ...
    def insert_hourly_production(self, production):
        cur = self.mysql.connection.cursor()
        cur.execute(
            "INSERT INTO hourly_production (panel_id, timestamp, energy_produced) VALUES (%s, %s, %s)",
    ...
    def update_hourly_production(self, production_id, production):
        cur = self.mysql.connection.cursor()
        cur.execute(
            "UPDATE hourly_production SET panel_id = %s, timestamp = %s, energy_produced = %s WHERE id = %s",

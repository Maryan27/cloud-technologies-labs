class BatteryChargeLevelDAO:
    ...
    def insert_charge_level(self, charge_level):
        cur = self.mysql.connection.cursor()
        cur.execute(
            "INSERT INTO battery_charge_level (battery_id, charge_level, timestamp) VALUES (%s, %s, %s)",
    ...
    def update_charge_level(self, charge_level_id, charge_level):
        cur = self.mysql.connection.cursor()
        cur.execute(
            "UPDATE battery_charge_level SET battery_id = %s, charge_level = %s, timestamp = %s WHERE id = %s",
